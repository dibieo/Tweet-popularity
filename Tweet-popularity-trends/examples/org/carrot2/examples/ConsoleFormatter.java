...
public class ConsoleFormatter
{
    ...
    private static void displayDocument(final int level, Document document)
    {
        ...
        System.out.printf(indent + "[%2d]Tweet id: ", document.getId());
        System.out.println(document.getField(Document.TITLE));
        final String url = document.getField(Document.CONTENT_URL);
        final String content = document.getField(Document.SUMMARY);
        System.out.println(indent + "Tweet content : " + content);
        if (StringUtils.isNotBlank(url))
        {
            System.out.println(indent + "Geo-code: " + url);
        }
        System.out.println();
    }
